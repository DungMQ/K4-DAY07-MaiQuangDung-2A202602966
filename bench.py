...
def main():
    ...
    try:
        ...
        print("[*] Embedding Backend: OpenAI text-embedding-3-small (có cache disk)")
    except Exception as e:
        print(f"[*] Fallback Embedder ({e}): MockEmbedder")
    ...
    output_lines.append("Mô hình embedding: OpenAI text-embedding-3-small\n")
    ...
    for b in BENCHMARK_QUERIES:
        ...
        # Agent trả lời
        if meta_filter:
            agent_answer = agent.answer_with_filter(qtext, metadata_filter=meta_filter, top_k=3)
        else:
            agent_answer = agent.answer(qtext, top_k=3)
